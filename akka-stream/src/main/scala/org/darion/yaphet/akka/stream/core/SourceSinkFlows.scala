...
import akka.stream.scaladsl.{BroadcastHub, Flow, GraphDSL, Keep, MergeHub, RunnableGraph, Sink, Source}

import scala.concurrent.{Future, Promise}

object SourceSinkFlows {
  def main(args: Array[String]): Unit = {
    implicit val system = ActorSystem("SourceSinkFlows")
    ...
    val (sink0, source0) = MergeHub.source[String](perProducerBufferSize = 16)
